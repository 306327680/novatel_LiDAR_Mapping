...
void gpsTools::updateGPSpose(Eigen::Vector3d gps_msgs) {
    if(lla_origin_ == Eigen::Vector3d::Identity()){
        Eigen::Vector3d lla  = gps_msgs;
        lla_origin_ = lla;
        gps_pos_ = Eigen::Vector3d::Identity();

        std::string zone; //unused
        gps_common::LLtoUTM(lla[0], lla[1], log_init , lat_init , zone);
        hgt_init = lla[2];
    } else{
        Eigen::Vector3d lla = gps_msgs;
      /*  Eigen::Vector3d ecef = LLA2ECEF(lla);
        ...
        gps_pos_ = enu;*/
        std::string zone; //unused
        gps_common::LLtoUTM(lla[0], lla[1], gps_pos_(1) , gps_pos_(0) , zone);
        gps_pos_(0) = gps_pos_(0)-lat_init;
        gps_pos_(1) = gps_pos_(1)-log_init;
        gps_pos_(2) = lla(2)-hgt_init;
    }
}
